...
def train_loop(
    ...
    SAVE_ROOT = os.path.join(os.getcwd(), 'saved/') if save_root is None else save_root
    PARAMS_FILE = os.path.join(SAVE_ROOT, 'params.ckpt')
    TRAIN_LOOP_FILE = os.path.join(SAVE_ROOT, 'train_loop.pkl', )
    TRAIN_OUTPUT_FILE = os.path.join(SAVE_ROOT, 'train_output.ndjson')
    TRAIN_COSTS_FILE = os.path.join(SAVE_ROOT, 'train_costs')
    ...
    if os.path.isfile(TRAIN_LOOP_FILE):
        ...
        with open(TRAIN_LOOP_FILE, 'rb') as f:
            _vars = pickle.load(f, encoding='bytes')
    ...
    def log(outputs, test, _vars, extra_things_to_print):
        ...
        for k, v in extra_things_to_print:
            entry[k] = v
        ...
        if 'train cost' in entry:
            train_costs.append(entry['train cost'])
    ...
    def save_train_output_and_params(iteration):
        ...
        np.savetxt(TRAIN_COSTS_FILE, np.array(train_costs))
    ...
    while True:
        ...
        if ((test_data is not None) and _vars['iteration'] % test_every == (test_every - 1)) or (
                (callback is not None) and _vars['iteration'] % callback_every == (callback_every - 1)):
            if inject_iteration:
